  ...
  protected void census(int population[]) {
    ...
    for (int r = row - 1; r <= row + 1; r++) {
      for (int c = col - 1; c <= col + 1; c++) {
        if (r >= 0 && c >= 0 && r < this.plain.getWidth() && c < this.plain.getWidth()) {

          if (this.plain.grid[r][c].who() == State.BADGER) {
            population[BADGER] += 1;
          } else if (this.plain.grid[r][c].who() == State.EMPTY) {
            population[EMPTY] += 1;
          } else if (this.plain.grid[r][c].who() == State.FOX) {
            population[FOX] += 1;
          } else if (this.plain.grid[r][c].who() == State.GRASS) {
            population[GRASS] += 1;
          } else {
            population[RABBIT] += 1;
          }
